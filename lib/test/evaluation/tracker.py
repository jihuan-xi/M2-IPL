...
import numpy as np
import json
from lib.models.layers.mqa import get_input_state
...
class Tracker:
    ...
    def __init__(self, name: str, parameter_name: str, dataset_name: str, run_id: int = None, display_name: str = None,
                 result_only=False, checkpoint_path=None, mqa_checkpoint_path=None, debug=False, update=[1., 0., 0.]):
        # runid改成要测试的epoch
        ...
        self.mqa_checkpoint_path = None
        # self.mqa_checkpoint_path = mqa_checkpoint_path
        ...
        if self.run_id is None:
            ...
            if self.update:
                self.results_dir = '{}/{}/{}_ep{:03d}_{}'.format(env.results_path, self.name, self.parameter_name,
                                                                 self.run_id,
                                                                 str(update)[1:-1].replace(', ', '').replace('0', ''))
            else:
                self.results_dir = '{}/{}/{}_ep{:03d}'.format(env.results_path, self.name, self.parameter_name,
                                                              self.run_id)
        if result_only:
    ...
    def _track_sequence(self, tracker, seq, init_info):
        ...
        for frame_path_v, frame_path_i in zip(seq.frames_v[1:], seq.frames_i[1:]):
            ...
            if 'miss' in self.dataset_name:
                ...
                image_v = self._read_image(frame_path_v)  # load each frame image
                image_i = self._read_image(frame_path_i)
                input_state = get_input_state(image_v, image_i)
            ...
            # if tracker.mqa:
            #     tracker.mqa.reset_buffer()
            out = tracker.track(image_v, image_i, info, input_state, out_prev, seq_name=seq.name)
    ...
    def get_parameters(self, update: list):
        ...
        if not self.update:
            params = param_module.parameters(self.parameter_name, self.checkpoint_path,
                                             self.mqa_checkpoint_path)  # 不采用模板更新
        else:
            params = param_module.parameters(self.parameter_name, self.checkpoint_path,
                                             self.mqa_checkpoint_path, update)  # 采用模板更新
        return params
