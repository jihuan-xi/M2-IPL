...
class OSTrack_twobranch(nn.Module):
    ...
    def forward(self, template: torch.Tensor,
        ...
        x_tir_fake, aux_dict_tir_fake, feat_layer_list_realrgb2tir_fake, feat_layer_list_faketir2rgb_fake = self.backbone(
            z=template[0], x=search[0],
            ce_template_mask=ce_template_mask,
            ce_keep_rate=ce_keep_rate,
            return_last_attn=return_last_attn, input_state='rgb2tir')

        ################################### input tir2rgb modality##################################
        x_rgb_fake, aux_dict_rgb_fake, feat_layer_list_realtir2rgb_fake, feat_layer_list_fakergb2tir_fake = self.backbone(
            z=template[1], x=search[1],
            ce_template_mask=ce_template_mask,
            ce_keep_rate=ce_keep_rate,
            return_last_attn=return_last_attn, input_state='tir2rgb')
