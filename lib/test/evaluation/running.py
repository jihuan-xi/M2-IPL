...
def _save_tracker_output(seq: Sequence, tracker: Tracker, output: dict):
    ...
    if seq.dataset in ['rgbt234', 'gtot', 'rgbt210', 'lasher', 'lashertestingset', 'rgbt234_miss',
                       'lashertestingset_miss', 'vtuav_miss', 'urvis']:
    ...
    def save_bb(file, data):
        ...
        if seq.dataset in ['urvis']:
            tracked_bb = tracked_bb.astype(float)
            np.savetxt(file, tracked_bb, delimiter=' ')
            return
        tracked_bb = tracked_bb.astype(int)
    ...
    for key, data in output.items():
        ...
        elif key == 'time' and seq.dataset not in ['urvis']:
