...
from lib.models.layers.mqa import MQAModule
from lib.models.layers.kalman import KalmanFilter, FeatureValidator
...
class IPLTrack(BaseTracker):
    def __init__(self, params, dataset_name):
        ...
        # self.mqa = None
        self.kalman = None
        self.base_search_scale = self.params.search_factor
        self.search_scale = self.params.search_factor
        self.max_search_scale = 6.5
    ...
    def initialize(self, image_v, image_i, info: dict):
        ...
        z_patch_arr_rgb, resize_factor_rgb, z_amask_arr_rgb = sample_target(image_v, info['init_bbox'],
                                                                            self.params.template_factor,
                                                                            output_sz=self.params.template_size)

        z_patch_arr_tir, resize_factor_tir, z_amask_arr_tir = sample_target(image_i, info['init_bbox'],
                                                                            self.params.template_factor,
                                                                            output_sz=self.params.template_size)
        ...
        self.frame_count = 0
        self.kalman = KalmanFilter(self.state)
        ...
        if self.save_all_boxes:
            ...
            return {"target_bbox": self.state, "current_max_score": 0.95}

    def track(self, image_v, image_i, info: dict = None, input_state=None, prev_output=None, seq_name=None):
        # if self.mqa:
        #     input_state = self.mqa.forward(image_v, image_i)

        if self.frame_count < 1:
            logger.info(seq_name)

        response_b, rgb_b, tir_b = None, None, None
        x_patch_arr_rgb, x_patch_arr_tir = None, None
        if input_state == 'rgbtir':
            H, W, _ = image_v.shape
            self.frame_id += 1
            self.frame_count += 1
            # (x1, y1, w, h)
            x_patch_arr_rgb, resize_factor_rgb, x_amask_arr_rgb = sample_target(image_v, self.state,
                                                                                # self.params.search_factor,
                                                                                self.search_scale,
                                                                                output_sz=self.params.search_size)
            # (x1, y1, w, h)
            x_patch_arr_tir, resize_factor_tir, x_amask_arr_tir = sample_target(image_i, self.state,
                                                                                # self.params.search_factor,
                                                                                self.search_scale,
                                                                                output_sz=self.params.search_size)
            ...
            pred_box = (pred_boxes.mean(dim=0) * self.params.search_size / resize_factor_rgb) \
                .tolist()  # (cx, cy, w, h) [0,1]
            # get the final box result
            self.state = clip_box(self.map_box_back(pred_box, resize_factor_rgb), H, W, margin=10)

            response_b = response
            # rgb_b, tir_b = out_dict['feat_layer_list']
            ...
            # Baseline: Take the mean of all pred boxes as the final result
            # (cx, cy, w, h) [0,1]
            pred_box = (pred_boxes.mean(dim=0) * self.params.search_size / resize_factor_tir).tolist()
            # get the final box result
            ...
            self.frame_count += 1
            pre_bbox = prev_output['target_bbox']
            pre_score = prev_output['current_max_score']

            pred_bbox, uncertainty = self.kalman.predict_with_uncertainty()
            alpha = max(0.9, pre_score)
            self.state = [alpha * pre_bbox[i] + (1 - alpha) * pred_bbox[i] for i in range(len(pre_bbox))]
            search_expansion_factor = uncertainty * 0.06
            self.search_scale = min(self.base_search_scale + search_expansion_factor, self.max_search_scale)
            response_b = torch.Tensor([0.0])

        if input_state is not 'skip':
            current_max_score = response_b.max().item()
            self.kalman.update(self.state, current_max_score)
            self.search_scale = self.base_search_scale

            valid, _, _ = self.feature_validator.varify_proposal(
                 [out_dict['backbone_feat_rgb'], out_dict['backbone_feat_tir']], self.state, input_state
            )
            if current_max_score > 0.8 and valid:
                box_mask_z_rgb = self.box_mask_z_rgb
                box_mask_z_tir = self.box_mask_z_tir
                template_rgb = self.z_dict1_rgb
                template_tir = self.z_dict1_tir

                if input_state in ['rgbtir', 'rgb']:
                    z_patch_arr_rgb, z_resize_factor_rgb, z_amask_arr_rgb = sample_target(
                        image_v, self.state, self.params.template_factor,
                        output_sz=self.params.template_size
                    )
                    template_rgb = self.preprocessor.process(z_patch_arr_rgb, z_amask_arr_rgb)
                    if self.cfg.MODEL.BACKBONE.CE_LOC:
                        template_bbox_rgb = self.transform_bbox_to_crop(self.state, z_resize_factor_rgb,
                                                                        template_rgb.tensors.device).squeeze(1)
                        box_mask_z_rgb = generate_mask_cond(self.cfg, 1, template_rgb.tensors.device, template_bbox_rgb)

                    self.feature_validator.update_history_feature(
                        out_dict['backbone_feat_rgb'], self.state, current_max_score, input_state
                    )

                if input_state in ['rgbtir', 'tir']:
                    z_patch_arr_tir, z_resize_factor_tir, z_amask_arr_tir = sample_target(
                        image_i, self.state, self.params.template_factor,
                        output_sz=self.params.template_size
                    )
                    template_tir = self.preprocessor.process(z_patch_arr_tir, z_amask_arr_tir)
                    if self.cfg.MODEL.BACKBONE.CE_LOC:
                        template_bbox_tir = self.transform_bbox_to_crop(self.state, z_resize_factor_tir,
                                                                        template_tir.tensors.device).squeeze(1)
                        box_mask_z_tir = generate_mask_cond(self.cfg, 1, template_tir.tensors.device, template_bbox_tir)

                    self.feature_validator.update_history_feature(
                        out_dict['backbone_feat_tir'], self.state, current_max_score, input_state
                    )
                ...
                self.box_mask_z_rgb = box_mask_z_rgb
                self.box_mask_z_tir = box_mask_z_tir
                self.box_mask_z = [self.box_mask_z_rgb, self.box_mask_z_tir]
                self.box_mask_z = self.box_mask_z[0]

        # logger.info('{:04d}  {:6s}  {:.3f}'.format(self.frame_count, input_state, response_b.max().item()))
        ...
        if self.debug:
            if not self.use_visdom:
                ...
                if 1:
                    ...
                    st_12_tir[out_dict['aux_dict_tir']['global_index_s'].squeeze().long()] = \
                        self.enc_attn_weights[-1][0].sum(0)[64:, :64].max(-1).values
                    # st_12_tir[out_dict['aux_dict_tir']['global_index_s'].squeeze().long()] = self.enc_attn_weights[-1][0].sum(0)[64:,:64].mean(-1)
                    st_12_tir = F.interpolate(st_12_tir.reshape(16, 16).unsqueeze(0).unsqueeze(0), scale_factor=8,
                                              mode='bilinear')
                    self.visdom.register(st_12_tir, 'heatmap', 1, 'attn-tir-st')

                    st_12_rgb = torch.zeros([256]).cuda()
                    st_12_rgb[out_dict['aux_dict_rgb']['global_index_s'].squeeze().long()] = self.enc_attn_weights[-13][
                                                                                                 0].sum(0)[64:,
                                                                                             :64].max(-1).values
                    # st_12_rgb[out_dict['aux_dict_rgb']['global_index_s'].squeeze().long()] = self.enc_attn_weights[-13][0].sum(0)[64:,:64].mean(-1)
                    st_12_rgb = F.interpolate(st_12_rgb.reshape(16, 16).unsqueeze(0).unsqueeze(0), scale_factor=8,
                                              mode='bilinear')
        ...
        if self.save_all_boxes:
            ...
            return {"target_bbox": self.state, "current_max_score": response_b.max().item()}
